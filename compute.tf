resource "aws_instance" "bastion" {
  ...
  subnet_id       = aws_subnet.kubespray-public[0].id
  security_groups = [aws_security_group.allow-ssh-from-bastion]
...
resource "aws_instance" "masters" {
  ...
  security_groups = [aws_security_group.allow-ips-to-bastion]
  key_name        = aws_key_pair.aws-pub-key.key_name
  tags = {
